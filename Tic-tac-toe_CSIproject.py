def ConstBoard(board):
  print("Current state of Board: \n\n");
  for i in range(0,9):
    if((i>0) and (i%3 == 0)):
      print("\n");
    if(board[i] == 0):
      print("_ ", end =" ");
    if(board[i] == -1):
      print("X ", end =" ");
    if(board[i] == 1):
      print("O ", end=" ");
  print("\n\n");

def User1Turn(board):
  pos = input("Enter X's position from [1,2,...,9]: ")
  pos = int(pos)
  if(board[pos-1] != 0):
    print("Invalid move!")
    exit(0)

  board[pos-1] = -1;

def User2Turn(board):
  pos = int(input("Enter O's position from [1,2,...,9]: "))
  if(board[pos-1]!=0):
    print("Invalid move!")
    exit(0)

  board[pos-1] = 1;

def minmax(board, player):
  x = analyzeBoard(board);
  if(x!=0):
    return (x*player);
  pos = -1;
  value = -2;
  for i in range(0,9):
    if(board[i] == 0):
      board[i] = player;
      score =-minmax(board, player*-1);
      board[i] = 0;
      if(score>value):
        value = score;
        pos = i;
  if(pos == -1):
    return 0;
  return value;

def CompTurn(board):
  pos = -1;
  value = -2
  for i in range(0,9):
    if(board[i] == 0):
      board[i] = 1
      score = -minmax(board, -1)
      board[i] = 0
      if(score>value):
        value = score
        pos = i

  board[pos] = 1;  

def analyzeBoard(board):
  match = [[0,1,2],[0,3,6],[0,4,8],[2,5,8],[2,4,6],[6,7,8],[3,4,5],[1,4,7]]
  for i in range(0,8):
    if(board[match[i][0]] != 0 and board[match[i][1]] == board[match[i][0]] and board[match[i][2]] == board[match[i][0]]):
      return board[match[i][0]]

  return 0;
player = 0
def main():
  choice = int(input("Enter 1 for single player, 2 for Multiplayer: "));
  board = [0,0,0,0,0,0,0,0,0]
  if(choice == 1):
    print("You: X vs Computer: O");
    player = int(input("Enter if you want to play 1(st) or 2(nd): "));
    for i in range(0,9):
      if(analyzeBoard(board) != 0):
        break;
      if((i+player)%2 == 0):
        CompTurn(board);
      else:
        ConstBoard(board);
        User1Turn(board);
  else:
    for i in range(0,9):
      if(analyzeBoard(board) != 0):
        break;
      if(i%2 == 0):
        ConstBoard(board);
        User1Turn(board);
      else:
        ConstBoard(board);
        User2Turn(board);

  result = analyzeBoard(board)
  if(result == 0):
    ConstBoard(board)
    print("Game Draw!")
  if(result == 1):
    ConstBoard(board)
    print("O wins!")
  if(result == -1):
    ConstBoard(board)
    print("X wins!");
main()